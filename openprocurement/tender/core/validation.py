    ...
    def award_valid(request, awardID, additional=False):
        if request.context.status != 'active' and 'status' in data and data['status'] == 'active':
            ...
            if stand_still_end > get_now():
                raise_operation_error(request, 'Can\'t sign contract before stand-still{} period end ({})'.format(
            ...
            if pending_complaints or pending_awards_complaints:
                raise_operation_error(request, 'Can\'t sign contract before reviewing all{} complaints'.format(
