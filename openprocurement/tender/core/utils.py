...
def check_merged_contracts(request):
    ...
    if 'additionalAwardIDs' in contract:
        # Get ids for all previous merged contracts
        old_additional_award_ids = contract.get('additionalAwardIDs', [])
        new_additional_award_ids = data['additionalAwardIDs']
        prev_contracts = [
            prev_contract for prev_contract in tender['contracts']
            if prev_contract['awardID'] in old_additional_award_ids
        ]
        new_contracts = [
            new_contract for new_contract in tender['contracts']
            if new_contract['awardID'] in new_additional_award_ids
        ]
        ...
        for new_contract in new_contracts:
            ...
            if new_contract['status'] != 'pending' and new_contract not in prev_contracts:
                request.errors.add('body', 'data', 'Can\'t merge contract in status {}'.format(new_contract['status']))
