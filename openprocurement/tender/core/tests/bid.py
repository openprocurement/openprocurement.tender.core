...
class TenderBidResourceTest(TenderContentWebTest):
    ...
    def test_create_tender_bid(self):
        ...
        self.assertEqual(response.content_type, 'application/json')
        self.assertIn('Bid can be added only during the tendering period',
    ...
    def test_patch_tender_bid(self):
        ...
        response = self.app.patch_json('/tenders/{}/bids/{}?acc_token={}'.format(
            self.tender_id, bid['id'], token), {"data": {"value": {"amount": 400}}}, status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertIn('Bid can be updated only during the tendering period',
                      response.json['errors'][0]["description"])
    ...
    def test_get_tender_bid(self):
        ...
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertIn('Bid can be deleted only during the tendering period',
                      response.json['errors'][0]["description"])
...
class TenderBidDocumentResourceTest(TenderContentWebTest):
    ...
    def test_create_tender_bid_document(self):
        ...
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertIn('Document can be added only during the tendering period',
                      response.json['errors'][0]["description"])
    ...
    def test_put_tender_bid_document(self):
        ...
        response = self.app.put('/tenders/{}/bids/{}/documents/{}?acc_token={}'.format(
            self.tender_id, self.bid_id, doc_id, self.bid_token), upload_files=[('file', 'name.doc', 'content')], status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertIn('Document can be updated only during the tendering period',
                      response.json['errors'][0]["description"])
